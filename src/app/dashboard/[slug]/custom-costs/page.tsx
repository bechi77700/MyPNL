        ...
        <Carte className="overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {charges.map((c) => (
                  <tr key={c.id} className="border-t border-bord transition hover:bg-carte-haut/50">
                    <td className="px-5 py-2.5 text-texte">{c.label}</td>
                    <td className="px-3 py-2.5 text-doux">
                    ...
                    <td className="chiffres px-3 py-2.5 text-faible">{c.effective_from}</td>
                    <td className="px-5 py-2.5 text-right">
                      <form action={supprimerCharge.bind(null, slug, c.id)}>
                        <button className="text-xs text-faible transition hover:text-negatif">
                          Supprimer
                        </button>
                      </form>
                    </td>
      ...
      <Carte className="mt-4 px-5 py-5">
        <form action={ajouterCharge.bind(null, slug)} className="flex flex-wrap items-end gap-2.5">
          <label className="flex flex-col gap-1.5">
            <span className="text-xs text-faible">Libellé</span>
            <Champ name="label" required placeholder="Klaviyo" className="w-44" />
          </label>
          <label className="flex flex-col gap-1.5">
            <span className="text-xs text-faible">Catégorie</span>
            <select name="category" className={cls}>
              {CATEGORIES.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
          </label>
          <label className="flex flex-col gap-1.5">
            <span className="text-xs text-faible">Montant ({devise})</span>
            <Champ name="amount" type="number" step="0.01" min="0" required className="chiffres w-28 text-right" />
          </label>
          <label className="flex flex-col gap-1.5">
            <span className="text-xs text-faible">Type</span>
            <select name="kind" className={cls}>
              {TYPES.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
          </label>
          <label className="flex flex-col gap-1.5">
            <span className="text-xs text-faible">Depuis</span>
