-- ============================================================
-- 024 — Granularite trimestrielle pour le P&L
-- ============================================================
...
language sql stable as $$
  ...
  fusion as (
    select j.*, coalesce(a.charges, 0) as charges, coalesce(a.remuneration, 0) as remuneration
    from j left join alloc a on a.day = j.day
  )
  select
    case p_grain
      when 'month'   then date_trunc('month', day)::date
      when 'quarter' then date_trunc('quarter', day)::date
      else day end,
    ...
    sum(charges) + sum(disputes_lost),
    sum(remuneration),
    sum(contribution) - (sum(charges) + sum(disputes_lost)),
